      ...
      <div className="max-w-6xl mx-auto px-5 py-4 flex items-center justify-between gap-3">
        <Link to="/" className="flex items-center gap-2.5 group shrink-0">
          <motion.div whileHover={{ rotate: 12 }}
            className="h-9 w-9 rounded-xl grad-primary glow grid place-items-center text-primary-foreground font-black">⛽</motion.div>
          <div className="hidden sm:block">
            <div className="text-sm font-bold tracking-tight">FuelDesk</div>
            <div className="text-[10px] text-muted-foreground tracking-widest uppercase">
              {displayName ? displayName : "Smena hisob-kitobi"}
            </div>
          </div>
        </Link>
        <div className="flex items-center gap-2">
            ...
            {tabs.map((t) => {
              ...
              return (
                <Link key={t.to} to={t.to}
                  ...
                  {active && (
                    <motion.div layoutId="nav-pill" className="absolute inset-0 grad-primary rounded-full"
                      transition={{ type: "spring", stiffness: 400, damping: 30 }} />
                  )}
                  <span className={`relative ${active ? "text-primary-foreground" : "text-muted-foreground"}`}>{t.label}</span>
                </Link>
              );
            })}
          </nav>
          ...
          {session && (
            <button
              ...
              title="Chiqish"
              className="h-9 w-9 grid place-items-center rounded-xl border border-border/60 hover:border-destructive/50 hover:text-destructive transition-colors text-sm">
              ⎋
            </button>
          )}
        </div>
